          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: 'var(--bg-hover)' }}>
                {['#', 'Cliente', 'Descrição', 'Status', 'Prioridade', 'Prazo', 'Valor', ...((cargo === 'ADMIN' || cargo === 'SOCIO') ? ['Ações'] : [])].map((col) => (
                  <th
                    key={col}
                    style={{
                      textAlign: 'left',
                        ...
                        <td style={{ padding: '12px 20px' }} onClick={e => e.stopPropagation()}>
                          <button
                            onClick={async () => {
                              ...
                              const r = await fetch(`/api/pedidos/${pedido.id}`, { method: 'DELETE' })
                              if (r.ok) carregarPedidos()
                            }}
                            style={{ padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontFamily: 'Inter, sans-serif', border: '1px solid var(--red-light)', backgroundColor: 'transparent', color: 'var(--red)', cursor: 'pointer' }}
                          >
                            Excluir
                          </button>
                        </td>
